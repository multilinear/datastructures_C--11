...
class TreeHashTableNode {
	public:
		size_t hs;
		Data_T data;
		TreeHashTableNode() {}
		TreeHashTableNode(const Data_T& set_data, size_t set_hs) {
			hs = set_hs;
			data = set_data;
		}
};
...
class TreeHashTableComp {
  public:
    ...
    static void printT(const TreeHashTableNode<Data_T> &n) {
      HC::printT(n.data);
    }
};
...
class TreeHashTable {
  private:
    Array<BTree<TreeHashTableNode<Data_T>, Val_T, TreeHashTableComp<Data_T,Val_T, HC>, ARITY>> table;
    size_t count = 0;
    void check_sizeup(void);
    void check_sizedown(void);
  public:
    TreeHashTable();
    TreeHashTable(size_t s);
    ~TreeHashTable();
    bool insert(const Data_T& data);
    Data_T* get(Val_T key);
    void remove(Val_T key);
...
template <typename Data_T, typename Val_T, typename HC>
TreeHashTable<Data_T, Val_T, HC>::TreeHashTable():table(MINSIZE) {
  // We have to initialize the lists since
  // array doesn't construct objects it contains
  for (size_t i=0; i<MINSIZE; ++i) {
    table[i] = BTree<TreeHashTableNode<Data_T>, Val_T, TreeHashTableComp<Data_T,Val_T, HC>, ARITY>();
  }
}

template <typename Data_T, typename Val_T, typename HC>
TreeHashTable<Data_T, Val_T, HC>::TreeHashTable(size_t s):table(s) {
  // We have to initialize the lists since
  // array doesn't construct objects it contains
...
template <typename Data_T, typename Val_T, typename HC>
TreeHashTable<Data_T, Val_T, HC>::~TreeHashTable() {
...
template <typename Data_T, typename Val_T, typename HC>
bool TreeHashTable<Data_T, Val_T, HC>::insert(const Data_T& data) {
  check_sizeup();
  Val_T v = HC::val(data);
  size_t i = HC::hash(v) % table.len();
  if (table[i].insert(TreeHashTableNode<Data_T>(data, table.len()))) {
    count++;
    return true;
  }
  return false;
...
template <typename Data_T, typename Val_T, typename HC>
void TreeHashTable<Data_T, Val_T, HC>::remove(Val_T v) {
  // Note, if n is not in the hashtable, this will cause
  // some nasty corruption.
  size_t i = TreeHashTableComp<Data_T,Val_T, HC>::hash(v) % table.len();
  TreeHashTableNode<Data_T> tmp;
...
template <typename Data_T, typename Val_T, typename HC>
bool TreeHashTable<Data_T, Val_T, HC>::isempty(void) const {
  return count == 0;
}

template <typename Data_T, typename Val_T, typename HC>
void TreeHashTable<Data_T, Val_T, HC>::resize(size_t s) {
  // nothing to do
  if (s == table.len()) {
    return;
  }
  ...
  while (i<old_size) {
    ...
    // remove it
    Val_T v = TreeHashTableComp<Data_T,Val_T,HC>::val(*n);
    TreeHashTableNode<Data_T> data;
    if (!table[i].remove(v, &data)) {
      PANIC("Value wasn't in Tree!\n"); 
    }
    // rehash it
    size_t new_index = TreeHashTableComp<Data_T,Val_T, HC>::hash(v) % s;
...
template <typename Data_T, typename Val_T, typename HC>
void TreeHashTable<Data_T, Val_T, HC>::check_sizedown(void) {
...
template <typename Data_T, typename Val_T, typename HC>
void TreeHashTable<Data_T, Val_T, HC>::check_sizeup(void) {
...
template <typename Data_T, typename Val_T, typename HC>
void TreeHashTable<Data_T, Val_T, HC>::print(void) {
	printf("[\n");
  for (size_t i=0; i<table.len(); ++i) {
		printf("  ");
    table[i].print();
