...
void check(BTreeHashTable<int, int, Comp> *hash, TrivialDict<TrivialDictDatum,int> *dict) {
  auto i = dict->begin();
  for (; i != dict->end(); i++) {
    if(!hash->get(i->val())) {
      printf("%d\n", i->val());
...
int main(int argc, char* argv[]) {
  ...
  printf("Begin BTreeHashTable.h test\n");
  int k;
